...
func TestLoadConfigDefaultsToDisabled(t *testing.T) {
	...
	config, err := LoadConfig("blueeconomy-administration-service")
	if err != nil {
	...
	if config.ServiceName != "blueeconomy-administration-service" {
...
func TestLoadConfigFailsClosedOnMalformedValues(t *testing.T) {
	...
	for _, testCase := range cases {
		t.Run(testCase.name, func(t *testing.T) {
			...
			if _, err := LoadConfig("blueeconomy-administration-service"); err == nil {
...
func TestMiddlewareCreatesSpanWithRouteAndStatus(t *testing.T) {
	...
	mux.HandleFunc("POST /v1/onboarding/requests/{id}/decision", func(writer http.ResponseWriter, request *http.Request) {
	...
	request := httptest.NewRequest(http.MethodPost, "/v1/onboarding/requests/req-1/decision", nil)
	...
	span := spans[0]
	if span.Name() != "POST /v1/onboarding/requests/{id}/decision" {
		t.Fatalf("span must be renamed to the matched route, got %q", span.Name())
	}
	attributes := span.Attributes()
	assertAttribute(t, attributes, "http.route", "POST /v1/onboarding/requests/{id}/decision")
